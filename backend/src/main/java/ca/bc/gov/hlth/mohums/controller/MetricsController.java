...
public class MetricsController {
    ...
    public List<Map<String, Object>> getActiveUserCount() throws SQLException {
        String sql
                ...
                + "       AND kcr.id IS NULL"
                + " )"
                + " WHERE NOT (client_id = 'account' AND (LOWER(realm_id) IN ('bceid_basic', 'bceid_business', 'bcprovider_aad', 'bcsc', 'bcsc_mspdirect', 'idir', 'idir_aad', 'master', 'moh_applications', 'moh_citizen', 'moh_idp', 'phsa'))"
    ...
    public Object getTotalNumberOfUsers() throws SQLException {
        String sql
                ...
                + "     WHERE (LOWER(realm_id) IN ('bceid_basic', 'bceid_business', 'bcprovider_aad', 'bcsc', 'bcsc_mspdirect', 'idir', 'mhsu_ehs', 'moh_idp', 'phsa'))"
    ...
    public List<Map<String, Object>> getUniqueUserCountByIDP() throws SQLException {
        String sql
                ...
                + "    SELECT DISTINCT id, realm_id"
                + "      FROM keycloak.user_entity"
                + "     WHERE LOWER(realm_id) IN ('bceid_basic', 'bceid_business', 'bcprovider_aad', 'bcsc', 'bcsc_mspdirect', 'idir', 'idir_aad', 'moh_idp', 'phsa')"
                + "       AND enabled = 1"
                + " )"
                + " GROUP BY realm_id"
    ...
    public List<Map<String, Object>> getUniqueUserCountByRealm() throws SQLException {
        String sql
                ...
                + "    SELECT DISTINCT id, realm_id"
                + "      FROM keycloak.user_entity"
                + "     WHERE LOWER(realm_id) NOT IN ('bceid_basic', 'bceid_business', 'bcprovider_aad', 'bcsc', 'bcsc_mspdirect', 'idir', 'idir_aad', 'master', 'moh_idp', 'phsa')"
                + "       AND enabled = 1"
                + " )"
                + " GROUP BY realm_id"
                + " ORDER BY realm_id ASC";
