...
public interface EventRepository extends JpaRepository<EventEntity, String> {
    ...
    @Query(value = """
            SELECT ee.USER_ID AS userId, MAX(ee.EVENT_TIME) AS lastLogin FROM KEYCLOAK.EVENT_ENTITY ee
             WHERE ee.REALM_ID = 'moh_applications' AND ee.type = 'LOGIN' AND ee.EVENT_TIME > EXTRACT(EPOCH FROM (CURRENT_DATE - INTERVAL '365 days')) * 1000
    ...
    @Query(value = "SELECT ee.USER_ID AS userId, MAX(ee.EVENT_TIME) AS lastLogin FROM KEYCLOAK.EVENT_ENTITY ee" +
            ...
            "AND ee.EVENT_TIME > EXTRACT(EPOCH FROM (CURRENT_DATE - INTERVAL '365 days')) * 1000 AND ee.CLIENT_ID = ?2" +
            " GROUP BY ee.user_id HAVING MAX(ee.EVENT_TIME) < ?1",
            nativeQuery = true)
    ...
    @Query(value = "SELECT ue.id AS userId, 'Over a year ago' AS lastLogin FROM KEYCLOAK.USER_ENTITY ue  LEFT JOIN KEYCLOAK.EVENT_ENTITY ee ON ee.user_id = ue.id " +
            "WHERE ee.user_id IS NULL AND ue.created_timestamp < EXTRACT(EPOCH FROM (CURRENT_DATE - INTERVAL '365 days')) * 1000 AND ue.REALM_ID = 'moh_applications'",
            nativeQuery = true)
